...
from Algorithms.pytorch.Models.MultiOneHotCategorical import MultiOneHotCategorical
from Algorithms.pytorch.Models.MultiSoftMax import MultiSoftMax
...
class Policy(BasePolicy):
    ...
    def __init__(self, config=None):
        ...
        self.use_multivariate_distribution = config["use_multivariate_distribution"]
    ...
    def forward(self, x):
        ...
        if self.use_multivariate_distribution:
            dist_cont = MultivariateNormal(cont_mean, torch.diag_embed(cont_std))
        else:
    ...
    def get_log_prob(self, state, action):
        ...
        if self.use_multivariate_distribution:
            ...
            log_prob = dist_cont.log_prob(action[..., self.dim_disc_action:]).sum(dim=-1)
        if dist_disc:
            discrete_log_prob = dist_disc.log_prob(action[..., :self.dim_disc_action])
    ...
    def get_action_log_prob(self, states):
        ...
        if self.use_multivariate_distribution:
            log_prob = dist_cont.log_prob(action)  # use multivariate normal distribution
        else:
            log_prob = dist_cont.log_prob(action).sum(dim=-1)  # [batch_size]

        if dist_disc:
            discrete_action = dist_disc.sample()
            discrete_log_prob = dist_disc.log_prob(discrete_action)  # [batch_size]
