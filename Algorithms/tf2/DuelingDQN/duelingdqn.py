...
from Algorithms.tf2.DuelingDQN.duelingdqn_step import duelingdqn_step
from Algorithms.tf2.Models.QNet_duelingdqn import QNet_duelingdqn
...
class DuelingDQN:
    ...
    def _init_model(self):
        ...
        # initialize networks
        self.value_net = QNet_duelingdqn(num_states, self.num_actions)
        self.value_net_target = QNet_duelingdqn(num_states, self.num_actions)

        self.running_state = ZFilter((num_states,), clip=5)

        # load model if necessary
        if self.model_path:
            print("Loading Saved Model {}_dueling_dqn_tf2.p".format(self.env_id))
            self.running_state = pickle.load(open('{}/{}_dueling_dqn_tf2.p'.format(self.model_path, self.env_id), "rb"))
            self.value_net.load_weights("{}/{}_dueling_dqn_tf2".format(self.model_path, self.env_id))
    ...
    def choose_action(self, state):
        state = np.expand_dims(NDOUBLE(state), axis=0)
    ...
    def save(self, save_path):
        ...
        self.value_net.save_weights("{}/{}_dueling_dqn_tf2".format(save_path, self.env_id))